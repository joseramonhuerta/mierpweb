...
MySQL - 5.6.51-cll-lve : Database - erp_lagranbelleza
*********************************************************************
*/
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`erp_lagranbelleza` /*!40100 DEFAULT CHARACTER SET latin1 */;
...
) ENGINE=MyISAM AUTO_INCREMENT=10412 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=52014 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=5345 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=37389 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=55342 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=3029 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=49761 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=18346 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=856535 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=8718 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=5032 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=65772 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=5220 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=25508 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=228787 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=497433 DEFAULT CHARSET=utf8;
...
) ENGINE=MyISAM AUTO_INCREMENT=228860 DEFAULT CHARSET=utf8;
...
BEGIN
	...
	/*Globusman, caroline, barcelona, chromatic, pomania*/
	IF V_id_sucursal_origen = 2 then /*INTERNACIONAL, LAS QUE ESTAN EN ESTA LISTA NO SE SURTEN DE INTERNACIONAL*/
		...
		120/*DISAVA*/,63/*DUO*/,26/*EROS NAILS*/,20/*ESTEFANIA*/,59/*IMPORTACIONES GUADALAJARA*/,
		88/*INTMEX*/,37/*JAVIER MINA*/,40/*JDENIS*/,46/*JLASH*/,72/*JORDANA*/,52/*JULIA SANCHEZ*/,167/*L.A. COLORS*/,
		53/*LAKREEM*/,165/*NIN ACCESORIOS*/,47/*PACHS*/,
		112/*NIYA NAIL*/,136/*NUNN CARE*/,9/*ORGANIC NAIL*/,28/*OUTLET NAILS*/,60/*PERFECT NAIL*/,93/*PINK UP*/,153/*SANTILLAN*/,
		122/*SECRET COSMETICS*/,158/*STAR ONE*/, 117 /*mizz*/, 174 /*nai-nari*/,
		126 /*GOTT COLOR*/, 129 /*VOGLIA*/, 124 /*BLOOM*/, 194 /*BISSU FUNDAMENTALES*/, 193 /*PROSA*/, 176 /*VR NAILS*/, 185 /*obelli-na*/);
	end if;
	
	IF V_id_sucursal_origen = 3 THEN /*MAR, LAS QUE ESTAN EN ESTA LISTA SE SURTEN DEL MAR, LAS DEMAS SE ELIMINAN PARA SURTIRSE DE INTER*/
	...
	/*Agrego un producto por cada almacen existente para posteriormente sacar la existencia por cada almacen*/
	INSERT INTO TempProductosAlmacenes(id_producto ,id_empresa,id_sucursal,id_almacen,id_linea)	
	SELECT p.id_producto,a.id_empresa,a.id_sucursal,a.id_almacen,p.id_linea
	FROM cat_productos p, cat_almacenes a;
	
	/*Elimino los productos que no tuvieron venta en ese periodo, pero no me mostrara los productos que no tuvieron ventas pero que tienen existencia 0*/
	/*delete from TempProductosAlmacenes
	Where id_producto not in (Select distinct(id_producto) from TempVentas);
	*/
	
	/*inserto el listado de productos con su existencia en cada almacen, este es el proceso que hace que se trabe, cambiar por un update*/
	INSERT INTO TempProductos(id_producto ,id_empresa,id_sucursal,id_almacen,id_linea,stock_min,stock_max,stock)	
	SELECT id_producto ,id_empresa,id_sucursal,id_almacen,id_linea,0,0,0 FROM TempProductosAlmacenes;
	...
	/*despues de insertar todos los productos en la TempProductos con exsitencia 0, hacer un update con la tabla stocks para ver si es mas rapido*/	
	...
	SELECT s.id_producto ,p.id_empresa,p.id_sucursal,s.id_almacen,p.id_linea,s.stock_min,s.stock_max,s.stock
	FROM cat_productos_stocks s
	Inner Join TempProductos p ON p.id_producto = s.id_producto AND p.id_almacen = s.id_almacen;
	
	UPDATE TempProductos p
	INNER JOIN TempProductosAlmacenesStocks pp ON (p.id_producto = pp.id_producto AND pp.id_almacen = p.id_almacen)
	SET p.stock_min = IFNULL(pp.stock_min,0), p.stock_max = IFNULL(pp.stock_max,0), p.stock = IFNULL(pp.stock,0); 
	
	
	/*
	UPDATE TempProductos p		
		INNER JOIN cat_productos_stocks s ON p.id_producto = s.id_producto AND p.id_almacen = s.id_almacen
		SET p.stock_min = IFNULL(s.stock_min,0), p.stock_max = IFNULL(s.stock_max,0), p.stock = IFNULL(s.stock,0); 
...
/* Procedure structure for procedure `spReporteExistencia` */

/*!50003 DROP PROCEDURE IF EXISTS  `spReporteExistencia` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`mierp`@`%` PROCEDURE `spReporteExistencia`(V_id_almacen BIGINT,V_id_linea BIGINT)
BEGIN
	DROP TEMPORARY TABLE IF EXISTS TempProductos;
	...
	CREATE TEMPORARY TABLE TempProductos( 
		...
		codigo VARCHAR(200),			
		stock DECIMAL(24,6),
		precio_venta DECIMAL(24,6),
		total DECIMAL(24,6)
		
	); 
	
	DROP TEMPORARY TABLE IF EXISTS TempProductosStocks;
	
	CREATE TEMPORARY TABLE TempProductosStocks( 
		id_producto BIGINT,
		id_almacen BIGINT,				
		stock DECIMAL(24,6)
	); 	
	
	INSERT INTO TempProductos(id_producto,id_almacen,id_linea,descripcion,codigo_barras,codigo,precio_venta,stock)	
	SELECT p.id_producto,V_id_almacen,p.id_linea,p.descripcion,p.codigo_barras,p.codigo,precio_compra,0
	FROM cat_productos p WHERE p.status = 'A';
	/*left join cat_productos_stocks pp ON (p.id_producto = pp.id_producto AND pp.id_almacen = V_id_almacen);*/
	
	INSERT INTO TempProductosStocks(id_producto,id_almacen,stock)	
	SELECT id_producto,id_almacen,stock
	...
	update TempProductos p
	INNER JOIN TempProductosStocks pp ON (p.id_producto = pp.id_producto AND pp.id_almacen = p.id_almacen)
	set p.stock = pp.stock;	
	UPDATE TempProductos
	SET total = IF(IFNULL(stock,0) <= 0,0, (precio_venta * IFNULL(stock,0)));			
	
	SELECT p.id_producto,p.descripcion,p.codigo_barras,p.codigo,p.stock,l.nombre_linea,p.precio_venta,p.total	
	FROM TempProductos p
	INNER JOIN cat_lineas l ON l.id_linea = p.id_linea AND l.status = 'A'				
...
BEGIN
	...
	INSERT INTO TempVentas(id_producto,id_empresa,id_sucursal,id_linea,nombre_sucursal,descripcion,codigo_barras,codigo,nombre_linea,stock,tipo)		
	SELECT s.id_producto,su.id_empresa,su.id_sucursal,p.id_linea,su.nombre_sucursal,p.descripcion,p.codigo_barras,p.codigo,l.nombre_linea,SUM(IFNULL(s.stock,0)) AS 
	stock,1
	FROM cat_productos_stocks s
	...
	INNER JOIN cat_sucursales su ON su.id_sucursal = a.id_sucursal	
	GROUP BY s.id_producto,p.codigo,p.descripcion,su.id_sucursal,su.nombre_sucursal;
	
	UPDATE 	TempVentas tmp,
	(	
		SELECT vd.id_producto,v.id_sucursal,IFNULL(SUM(vd.cantidad),0) AS cantidad,IFNULL(MAX(vd.precio),0) AS precio, IFNULL(SUM(vd.total),0) AS subtotal FROM ventas_detalles vd
		INNER JOIN ventas v ON v.`id_venta` = vd.`id_venta`
		WHERE v.fecha_venta BETWEEN V_FechaInicio AND V_FechaFin AND v.status = 'A'	
		GROUP BY vd.id_producto,v.id_sucursal			
	) AS vtas
	SET tmp.cantidad = vtas.cantidad, tmp.precio = vtas.precio, tmp.subtotal = vtas.subtotal
	WHERE tmp.id_producto = vtas.id_producto AND tmp.id_sucursal = vtas.id_sucursal AND tmp.tipo=1;
	...
	UPDATE 	TempVentas tmp,
	(	
		SELECT vd.id_producto,v.id_sucursal,IFNULL(SUM(vd.cantidad),0) AS cantidad,IFNULL(MAX(vd.costo),0) AS precio, IFNULL(SUM(vd.total),0) AS subtotal FROM remisiones_detalles vd
		INNER JOIN remisiones v ON v.`id_remision` = vd.`id_remision`
		WHERE v.fecha BETWEEN V_FechaInicio AND V_FechaFin AND v.status = 'A'	
		GROUP BY vd.id_producto,v.id_sucursal			
	) AS vtas
	SET tmp.cantidad = vtas.cantidad, tmp.precio = vtas.precio, tmp.subtotal = vtas.subtotal
	WHERE tmp.id_producto = vtas.id_producto AND tmp.id_sucursal = vtas.id_sucursal AND tmp.tipo=2;
	...
	*/
	...
	SELECT t1.id_producto,t1.id_sucursal,t1.nombre_sucursal,t1.descripcion,t1.codigo_barras,t1.codigo,IFNULL(SUM(t1.cantidad),0) AS ventas,t1.nombre_linea,SUM(DISTINCT t1.stock) AS stock,MAX(t1.precio) AS precio, SUM(t1.subtotal) AS subtotal
	FROM  (
...
/* Procedure structure for procedure `spReporteVentasAnterior` */

/*!50003 DROP PROCEDURE IF EXISTS  `spReporteVentasAnterior` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`mierp`@`%` PROCEDURE `spReporteVentasAnterior`(V_id_empresa BIGINT,V_id_sucursal BIGINT,V_FechaInicio DATETIME, V_FechaFin DATETIME, V_id_linea BIGINT,V_id_producto BIGINT, V_Agrupado TINYINT)
BEGIN
	DROP TEMPORARY TABLE IF EXISTS TempVentas;
...
/* Procedure structure for procedure `spReporteVentasTodos` */

/*!50003 DROP PROCEDURE IF EXISTS  `spReporteVentasTodos` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`mierp`@`%` PROCEDURE `spReporteVentasTodos`(V_id_empresa BIGINT,V_id_sucursal BIGINT,V_FechaInicio DATETIME, V_FechaFin DATETIME, V_id_linea BIGINT,V_id_producto BIGINT, V_Agrupado TINYINT)
BEGIN
	DROP TEMPORARY TABLE IF EXISTS TempVentas;
	...
	*/
	...
	INSERT INTO TempResultado(id_producto,id_sucursal,nombre_sucursal,descripcion,codigo_barras,codigo,ventas,nombre_linea,stock,precio,subtotal)
	SELECT t1.id_producto,t1.id_sucursal,t1.nombre_sucursal,t1.descripcion,t1.codigo_barras,t1.codigo,SUM(t1.cantidad) AS ventas,t1.nombre_linea,SUM(DISTINCT t1.stock) AS stock,MAX(t1.precio) AS precio, SUM(t1.subtotal) AS subtotal
	FROM  (
		SELECT id_producto,id_sucursal,nombre_sucursal,descripcion,codigo_barras,codigo,cantidad,nombre_linea,stock,precio,subtotal FROM TempVentas order by id_sucursal
		) t1
		GROUP BY t1.id_producto,t1.codigo,t1.descripcion,t1.id_sucursal,t1.nombre_sucursal     
		ORDER BY t1.descripcion,t1.id_sucursal;
	...
	IF V_Agrupado = 1 THEN
		...
		FROM
		  (SELECT
		     nombre_sucursal
		   FROM     
		     TempResultado ORDER by id_sucursal
		) t;
		SET @PivotQuery = CONCAT('SELECT t1.codigo as CODIGO,t1.descripcion as DESCRIPCION,t1.nombre_linea as NOMBRE_LINEA,t1.codigo_barras AS CODIGO_BARRAS,t1.codigo AS CODIGO,SUM(t1.ventas) as VENTAS,', @PivotQuery, 
			' FROM TempResultado t1 group by t1.descripcion,t1.codigo_barras,t1.codigo,t1.nombre_linea ORDER BY t1.descripcion,t1.id_sucursal');
		PREPARE statement FROM @PivotQuery;
		EXECUTE statement;
		DEALLOCATE PREPARE statement;
	END IF;
