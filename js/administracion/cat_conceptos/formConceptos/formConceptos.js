 ...
 */
Ext.ns('miErpWeb');
formConceptos = Ext.extend(formConceptosUi, {
	inicializarStores: function(){
		this.cmbTipo.store = new miErpWeb.storeFormConceptosTipo();        
        var data=new Array(
        		{id:1,nombre:miErpWeb.formatearTexto('INGRESOS')},
        		{id:2,nombre:miErpWeb.formatearTexto('EGRESOS')}
        );
    ...
    initComponent: function() {
        formConceptos.superclass.initComponent.call(this);
		...
		this.txtIdConcepto.setValue=function(value){
			Ext.form.TextField.prototype.setValue.apply(this,arguments);
	...
	cancelar:function(){
		...
		Ext.Ajax.request({
			params: { 
				id_concepto: this.txtIdConcepto.getValue(),
				status:this.txtStatus.getValue()
			},
			scope:this,
		   	url: 'app.php/conceptos/cambiarstatus',
	...
	eliminar:function(btn){
		...
		this.el.mask(mew.mensajeDeEspera);
		Ext.Ajax.request({
			params: { id_concepto: this.txtIdConcepto.getValue() },
			scope:this,
		   	...
		   	success: function(response,options){	
				...
				this.fireEvent('eliminado',options.params.id_concepto);
	...
	load:function(){
		var params={idCon:this.txtIdConcepto.getValue()};
		this.el.mask(mew.mensajeDeEspera);
		this.getForm().load({
			params:params,
			url:'app.php/conceptos/obtenerconcepto',
			scope:this,
	...
	listeners:{
    	activate:function(){
    		...
    		if (this.idValue!=undefined && this.idValue!=0){
    			this.txtIdConcepto.setValue(this.idValue);
				this.load();
			} 
    	...
    	actioncomplete:function(form,action){
			...
			if (respuesta.success==true){
				var concepto = respuesta.data.Concepto;
				concepto.descripcion=miErpWeb.formatearTexto(concepto.descripcion);
										
				form.setValues(concepto);
...
Ext.reg('formConceptos', formConceptos);