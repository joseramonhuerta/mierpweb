 ...
 */
Ext.ns('miErpWeb');
formChequeras = Ext.extend(formChequerasUi, {
	configurarToolBar(){
    ...
    initComponent: function() {
        formChequeras.superclass.initComponent.call(this);
		...
		this.txtIdChequera.setValue=function(value){
			Ext.form.TextField.prototype.setValue.apply(this,arguments);
	...
	cancelar:function(){
		...
		Ext.Ajax.request({
			params: { 
				id_chequera: this.txtIdChequera.getValue(),
				status:this.txtStatus.getValue()
			},
			scope:this,
		   	url: 'app.php/chequeras/cambiarstatus',
	...
	eliminar:function(btn){
		...
		this.el.mask(mew.mensajeDeEspera);
		Ext.Ajax.request({
			params: { id_chequera: this.txtIdChequera.getValue() },
			scope:this,
		   	url: 'app.php/chequeras/eliminar',
		   	success: function(response,options){	
				var respuesta=Ext.decode(response.responseText);
				...
				this.fireEvent('eliminado',options.params.id_chequera);
	...
	load:function(){
		var params={idChe:this.txtIdChequera.getValue()};
		this.el.mask(mew.mensajeDeEspera);
		this.getForm().load({
			params:params,
			url:'app.php/chequeras/obtenerchequera',
			scope:this,
	...
	listeners:{
    	activate:function(){
    		...
    		if (this.idValue!=undefined && this.idValue!=0){
    			this.txtIdChequera.setValue(this.idValue);
				this.load();
			} 
    	...
    	actioncomplete:function(form,action){
			...
			if (respuesta.success==true){
				var chequera = respuesta.data.Chequera;
				chequera.descripcion=miErpWeb.formatearTexto(chequera.descripcion);
										
				form.setValues(chequera);
...
Ext.reg('formChequeras', formChequeras);