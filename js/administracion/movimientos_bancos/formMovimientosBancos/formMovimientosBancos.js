...
Ext.ns('miErpWeb');
Ext.ns('mew');
formMovimientosBancos = Ext.extend(formMovimientosBancosUi, {
	origen:1,//1=Movimientos Bancos,2=Gastos
    inicializarStores:function(){
		this.cmbTipoMovimiento.store = new miErpWeb.storeFormMovimientosBancosTiposMovimientos();        
        var data=new Array(
        		{id:1,nombre:miErpWeb.formatearTexto('INGRESO')},
        		{id:2,nombre:miErpWeb.formatearTexto('EGRESO')}
        );
		this.cmbTipoMovimiento.store.loadData({data:data});
		
		this.cmbTipoOrigen.store = new miErpWeb.storeFormMovimientosBancosTiposOrigen();        
        var dataTipoOrigen=new Array(
        		{id:1,nombre:miErpWeb.formatearTexto('EFECTIVO')},
        		{id:2,nombre:miErpWeb.formatearTexto('BANCOS')}
        );
		this.cmbTipoOrigen.store.loadData({data:dataTipoOrigen});
		
		
		this.cmbSerie.store =  new miErpWeb.storeFormMovimientosBancosSeries();
				
		this.cmbChequera.store =  new miErpWeb.storeFormMovimientosBancosChequeras();		
		
		this.cmbConcepto.store =  new miErpWeb.storeFormMovimientosBancosConceptos();
	...
	initComponent: function() {
        formMovimientosBancos.superclass.initComponent.call(this);
	...
	cargarDatos:function(data){
		...
		this.cmbTipoMovimiento.setValue(movimientobanco.tipo_movimiento);
		this.cmbTipoOrigen.setValue(movimientobanco.tipo_origen);

		if(movimientobanco.tipo_origen == 1)
			this.cmbChequera.setDisabled(true);
		else
			this.cmbChequera.setDisabled(false);
...
Ext.reg('formMovimientosBancos', formMovimientosBancos);