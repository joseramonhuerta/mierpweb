...
class ReporteVentasPDF extends FPDF{
	...
	function ReporteVentasPDF($orientation='L',$unit='mm',$format='Letter',$datos,$formatos){
	...
	public function Header() {
		...
		$this->Cell(100,10,utf8_decode("Reporte de Ventas"),0,0,'L');	
		$this->SetX(108);
	...
	function imprimeHeader(){
		...
		if ($this->imprimirHeader==true){
			...
			$this->Cell(27,5,utf8_decode("SUCURSAL"),$border,0,"C",$fill);
			$this->Cell(20,5,"SERIE-FOLIO",$border,0,'',$fill);			
			$this->Cell(25,5,utf8_decode("FECHA"),$border,0,"C",$fill);		//------Header 
			
			$this->Cell(39,5,utf8_decode("CLIENTE"),$border,0,"C",$fill);		//------Header
			$this->Cell(25,5,utf8_decode("VENDEDOR"),$border,0,"C",$fill);
			$this->Cell(18,5,utf8_decode("IMPORTE"),$border,0,"C",$fill);	
			$this->Cell(18,5,utf8_decode("DESC"),$border,0,"C",$fill);	
			$this->Cell(18,5,utf8_decode("TOTAL"),$border,0,"C",$fill);	
	...
	public function imprimeDetalles($datos){
		...
		foreach($datos['detalles'] as $dato){	
			...
			$Serie = $dato['serie_folio'];
				
			$Fecha = $dato['fecha_venta'];
			$Cliente = $dato['nombre_fiscal'];
			$Agente = $dato['nombre_agente'];
			$Sucursal = $dato['nombre_sucursal'];
			...
			$this->Cell(27,$heightCell,utf8_decode($Sucursal),$border,$saltoDeLinea,'',$fill);						
			$this->Cell(20,$heightCell,utf8_decode($Serie),$border,$saltoDeLinea,'',$fill);
					
			$this->Cell(25,$heightCell,utf8_decode($Fecha),$border,$saltoDeLinea,'',$fill);
			$this->Cell(39,$heightCell,utf8_decode($Cliente),$border,$saltoDeLinea,'',$fill);	
			$this->Cell(25,$heightCell,utf8_decode($Agente),$border,$saltoDeLinea,'',$fill);	
			$this->Cell(18,$heightCell,utf8_decode($Importe),$border,$saltoDeLinea,'R',$fill);	
			$this->Cell(18,$heightCell,utf8_decode($Descuento),$border,$saltoDeLinea,'R',$fill);			
	...
	function imprimirTotales($ultimo=false){
		...
		$importes = number_format ($datos['ventas']['importe'] ,$decimales ,  '.' , ',' );
		$descuento = number_format ($datos['ventas']['descuento'] ,$decimales ,  '.' , ',' );
		$subtotal = number_format ($datos['ventas']['subtotal'] ,$decimales ,  '.' , ',' );
		$impuestos = number_format ($datos['ventas']['impuestos'] ,$decimales ,  '.' , ',' );
		$total = number_format ($datos['ventas']['total'] ,$decimales ,  '.' , ',' );
