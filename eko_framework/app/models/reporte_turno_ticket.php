...
class ReporteTurnoTicket{
	...
	function crearReporte($datos,$formatos){
		...
		require "eko_framework/app/models/reporte_turno_ticket_pdf.php";
		

		$pdf=new ReporteTurnoTicketPDF('P','mm',array(250,70),$datos,$formatos);
		//--------------------------------------------------------------------------------------------------------
		$pdf->AddPage();
	...
	function obtenerDatos($params){
        ...
        $query = "SELECT t.id_turno,t.consecutivo,t.concepto,DATE_FORMAT(t.fechainicio,'%d/%m/%y %H:%i') as fecha,
		t.total_turno,s.nombre_sucursal
		FROM turnos t 
		...
		$query = "SELECT td.id_turno_detalle,f.nombre_formapago,d.denominacion,td.cantidad,td.total
		FROM turnos_detalles td 
