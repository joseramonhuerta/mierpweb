 ...
 */
class ReporteMovimientoCajaTicket{
	...
	function crearReporte($datos,$formatos){
		require "eko_framework/app/models/reporte_movimiento_caja_ticket_pdf.php";
		
		$pdf=new ReporteMovimientoCajaTicketPDF('P','mm',array(250,70),$datos,$formatos);
		$pdf->AddPage();
	...
	function obtenerDatos($params){
        ...
        $query = "SELECT a.id_movimiento_caja,a.concepto,DATE_FORMAT(a.fecha,'%d/%m/%y %H:%i') AS fecha,a.consecutivo,
		a.total,case a.tipo when 1 then 'DEPOSITO' when 2 then 'RETIRO' end as tipo,s.nombre_sucursal,a.total
		FROM movimientos_caja a
		INNER JOIN cat_sucursales s ON s.id_sucursal = a.id_sucursal
		WHERE a.id_movimiento_caja = $id";
		...
		$response=array();
      	$response['data']['movimientocaja']=$resArr[0]; 
