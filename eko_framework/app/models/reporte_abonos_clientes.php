...
class ReporteAbonosClientes{
	...
	function crearReporte($datos,$formatos){
		require "eko_framework/app/models/reporte_abonos_clientes_pdf.php";
		
		$pdf=new ReporteAbonosClientesPDF('P','mm','Letter',$datos,$formatos);
		$pdf->AddPage('P');
	...
	function obtenerDatos($params){
		...
		$filtroSql = " WHERE r.id_empresa = $idEmp AND a.fecha BETWEEN '$fechaInicio' AND '$fechaFin'";
        
		if (strlen($id_age) > 0) {
            $filtroSql.= " AND r.id_agente = $id_age ";
		}

		if (strlen($id_cli) > 0) {
            $filtroSql.=" AND r.id_cliente = $id_cli ";
		}
		...
		$resArrFiltros['fecha_inicio']  = $resArr[0]['fecha_inicio'];
		$resArrFiltros['fecha_fin']  = $resArr[0]['fecha_fin'];
		...
		$query = "SELECT DATE_FORMAT(a.fecha,'%d/%m/%Y') AS fecha, CONCAT(a.serie,' - ',a.folio) AS seriefolio, c.nombre_fiscal AS nombre_cliente,
					a.concepto, a.observacion, a.importe
					FROM cxc_abonos a
					INNER JOIN cxc cx ON cx.id_cxc = a.id_cxc
					INNER JOIN remisiones r ON r.id_remision = cx.id_remision
					INNER JOIN cat_clientes c ON c.id_cliente = r.id_cliente
					$filtroSql
					ORDER BY c.nombre_fiscal,a.fecha";
							
		//throw new Exception($query);		
		$resArrAbonos = $model->query($query);
		if ( empty($resArrAbonos) ){
			return array();
		}
		...
		$response['data']['abonos']=$resArrAbonos;
