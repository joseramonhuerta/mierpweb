...
class ReportePedidoSugerido{
	...
	function crearReporte($datos,$formatos){
		...
		require "eko_framework/app/models/reporte_pedido_sugerido_pdf.php";
		
		$pdf=new ReportePedidoSugeridoPDF('P','mm',array(250,70),$datos,$formatos);
		//--------------------------------------------------------------------------------------------------------
		$pdf->AddPage();
	...
	function obtenerDatos($params){
		...
		$idSucOrigen = $params['IDSucOrigen'];
		$idEmp = $params['IDEmp'];	  
		$idSuc = $params['IDSuc'];
		...
		$query ="CALL spPedidoSugerido($idSucOrigen,$idSuc,'$fechaInicio','$fechaFin',$id_lin,0);";
