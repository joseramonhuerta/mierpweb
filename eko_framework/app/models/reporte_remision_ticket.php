...
class ReporteRemisionTicket{
	...
	function crearReporte($datos,$formatos){
		...
		require "eko_framework/app/models/reporte_remision_ticket_pdf.php";
		
		$pdf=new ReporteRemisionTicketPDF('P','mm',array(250,70),$datos,$formatos);
		//--------------------------------------------------------------------------------------------------------
		$pdf->AddPage();
	...
	function obtenerDatos($params){
		...
		case r.aplicado when 1 then 'APLICADA' else 'NO APLICADA' end as aplicado		
		FROM remisiones r 		
