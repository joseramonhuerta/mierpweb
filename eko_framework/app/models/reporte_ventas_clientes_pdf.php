...
class ReporteVentasClientesPDF extends FPDF{
	var $cerrando=false;
	var $colorHeader;
	...
	function ReporteVentasClientesPDF($orientation='L',$unit='mm',$format='Letter',$datos,$formatos){
	...
	public function Header() {
		...
		$this->Cell(100,10,utf8_decode("Reporte Ventas de Clientes"),0,0,'L');	
		$this->SetX(108);
	...
	function imprimeHeader(){
		...
		if ($this->imprimirHeader==true){
			...
			$this->Cell(25,5,utf8_decode("CONDICION PAGO"),$border,0,"C",$fill);
			$this->Cell(50,5,utf8_decode("CONCEPTO"),$border,0,"C",$fill);		
			$this->Cell(53,5,utf8_decode("NOMBRE CLIENTE"),$border,0,"C",$fill);		//------Header			
	...
	public function imprimeDetalles($datos){
		...
		foreach($datos['remisiones'] as $dato){	
			...
			$CondicionPago = $dato['condicion_pago'];			
			$Abono = number_format($dato['total'] ,$decimales ,  '.' , ',' );
						
			
			$this->totalGeneralAbonos += $dato['total'];
			...
			$this->Cell(25,$heightCell,utf8_decode($CondicionPago),$border,$saltoDeLinea,'',$fill);	
			$this->Cell(50,$heightCell,utf8_decode($Concepto),$border,$saltoDeLinea,'',$fill);			
			$this->Cell(53,$heightCell,utf8_decode($NombreCliente),$border,$saltoDeLinea,'',$fill);			
