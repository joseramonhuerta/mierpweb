...
class ReporteInventario{
	...
	function crearReporte($datos,$formatos){
		require "eko_framework/app/models/reporte_inventario_pdf.php";
		
		$pdf=new ReporteInventarioPDF('P','mm',array(250,70),$datos,$formatos);
	...
	function obtenerDatos($params){
        ...
        $query = "
			SELECT m.id_inventario,m.serie_inventario,m.folio_inventario,DATE_FORMAT(m.fecha_inventario,'%d/%m/%y %H:%i') AS fecha_inventario,m.concepto_inventario,IFNULL(DATE_FORMAT(m.fecha_aplica,'%d/%m/%y %H:%i'),'') AS fecha_aplica,
			e.nombre_fiscal,s.nombre_sucursal,IFNULL(a.nombre_almacen,'') AS nombre_almacen
			FROM inventarios m 
		...
		$query = "SELECT d.id_inventario_detalle,p.codigo,p.codigo_barras,p.descripcion,u.codigo_unidad,d.stock,d.conteo,d.diferencia
		FROM inventarios_detalles d 
		...
		$response=array();
      	$response['data']['inventario']=$resArr[0]; 
