...
class ReporteCorteTicketPDF extends PDF_JavaScript{
	var $cerrando=false;
	var $colorHeader;
	...
	function ReporteCorteTicketPDF($orientation='L',$unit='mm',$format='Letter',$datos,$formatos){
	...
	public function Header() {
		...
		$nombre_fiscal = $datos['corte']['nombre_fiscal'];
		...
		$consecutivo=$datos['corte']['consecutivo'];
		$fecha_corte=$datos['corte']['fecha'];
		
		$concepto=$datos['corte']['concepto'];
		//----------------------------------------------------
		//			Titulo del reporte
		//----------------------------------------------------
		...
		$this->Ln();
		$this->SetX(2);
		$this->Cell(80,$hCell,mb_strtoupper(UTF8_decode("CIERRE DE TURNO")),$border,0,'L');	#	Valor
		$this->Ln();
		$this->SetX(2);
		$this->Cell(80,$hCell,mb_strtoupper(UTF8_decode($nombre_sucursal)),$border,0,'L');	#	Valor
		$this->Ln();
		$this->SetX(2);
		$this->Cell($ancho,$hCell,"FECHA HORA: ".mb_strtoupper(UTF8_decode($fecha_corte)) ,$border,0,'L');	
		$this->Ln();
		$this->SetX(2);
		$this->Cell(80,$hCell,mb_strtoupper(UTF8_decode($consecutivo)),$border,0,'L');	#	Valor		
		$this->Ln();
		$this->SetX(2);
	...
	function imprimeHeader(){
		...
		// if ($this->imprimirHeader==true){
			...
			$this->SetTextColor(0, 0, 0);
			$this->SetX(2);
			$this->Cell(41,2,"Detalles Liquidacion",$border,1,'L');
			$this->ln();			
			$fill=1;
			$this->SetX(2);
			...
			$this->Cell(31,5,"FORMA PAGO.",$border,0,"L",$fill);//------Header Impuestos
			$this->Cell(12,5,"DENOM.",$border,0,"R",$fill);//------Header Impuestos
			$this->Cell(12,5,"CANT",$border,0,"R",$fill);//------Header Impuestos
			$this->Cell(12,5,"TOTAL",$border,0,"R",$fill);//------Header Impuestos
	...
	function imprimeHeaderRetencion(){
		$border =0;
		// if ($this->imprimirHeader==true){
			//IMPRIMIR EGRESOS O INGRESOS
			$font="Arial";
			...
			$this->SetX(2);		
			$this->SetTextColor(0, 0, 0);
			$this->Cell(41,2,"Detalles Retencion",$border,1,'L');
			$this->ln();		
			$fill=1;
			$this->SetX(2);
			$wCliente=$this->wCliente;
	...
	public function imprimeDetallesLiquidacion($datos){
		$this->imprimeHeader();
		...
		foreach($datos['detallesLiquidacion'] as $dato){	
		$this->SetX(2);
        ...
        $this->SetTextColor(0, 0, 0);
		$this->imprimirTotalesLiquidacion();	
		// $hCell=5;
		// $PageBreakTrigger=$this->h-$this->bMargin;
		// $this->despuesdelosdetalles();
	}
	
	public function imprimeDetallesRetencion($datos){
		$this->imprimeHeaderRetencion();
		$decimales=$this->decimales;
		$datos=$this->datos['data'];
		$wCliente=65;
		$this->SetTextColor(0, 0, 0);
		...
		$fill = false; //Para alternar el relleno
		// for($i=0;$i<sizeof($datos);$i++){
		foreach($datos['detallesRetencion'] as $dato){	
		$this->SetX(2);
			// $this->Cell(15,5,utf8_decode($dato['cantidad']),0,0,'',$fill);
			$saltoDeLinea = 0;
			#		Primer linea
	...
	function imprimirTotalesLiquidacion($ultimo=false){
		...
		$total = number_format ($datos['corte']['total_liquidado'] ,$decimales ,  '.' , ',' );
		...
		$this->SetFont('Arial','',7);
		$this->SetX($columna);
		$this->Cell(12, $alto, "Total Liquidado:$", $border, 0, 'R');
	...
	function imprimirTotalesRetencion($ultimo=false){
		$this->aceptarSalto=false;
		$decimales=$this->decimales;	
		//$i=$this->PageNo();
		...
		$total = number_format ($datos['corte']['total_retenido'] ,$decimales ,  '.' , ',' );
		...
		$this->SetFont('Arial','',7);
		$this->SetX($columna);
		$this->Cell(12, $alto, "Total Retenido:$", $border, 0, 'R');
		$this->Cell(15, $alto, $total, $border, 1, 'R');
	...
	function imprimirTotalesCorte($ultimo=false){
		$this->aceptarSalto=false;
		$decimales=$this->decimales;	
		//$i=$this->PageNo();
		
		$datos=$this->datos['data'];
		$total_corte = number_format ($datos['corte']['total_corte'] ,$decimales ,  '.' , ',' );
		...
		$diferencia_corte = number_format ($datos['corte']['diferencia_corte'] ,$decimales ,  '.' , ',' );
		...
		$this->SetFont('Arial','',7);
		$this->SetX($columna);
		$this->Cell(12, $alto, "Total Corte:$", $border, 0, 'R');
		$this->Cell(15, $alto, $total_corte, $border, 1, 'R');
		$this->Ln();			
		$columna = 2;
		$this->SetX($columna);
		$this->Cell(66, $alto, "Resumen Corte:", $border, 0, 'L');
		
		$this->Ln();	
		$this->SetX($columna);
		$this->Cell(51, $alto, "Apertura:", $border, 0, 'L');
		$this->Cell(15, $alto, $total_turno, $border, 1, 'R');
		
		$this->SetX($columna);
		$this->Cell(51, $alto, "(+) Ventas:", $border, 0, 'L');
		$this->Cell(15, $alto, $total_ventas, $border, 1, 'R');
		...
		$this->SetX($columna);
		$this->Cell(51, $alto, "(-) Fondo Liquidado:", $border, 0, 'L');
		$this->Cell(15, $alto, $total_liquidado, $border, 1, 'R');
		
		$this->SetX($columna);
		$this->Cell(51, $alto, "(-) Fondo Retenido:", $border, 0, 'L');
		$this->Cell(15, $alto, $total_retenido, $border, 1, 'R');
		
		$this->SetX($columna);
		$this->Cell(51, $alto, "(=) Diferencia Corte +/-:", $border, 0, 'L');
		$this->Cell(15, $alto, $diferencia_corte, $border, 1, 'R');
