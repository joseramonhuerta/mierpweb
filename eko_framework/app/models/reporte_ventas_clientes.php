...
class ReporteVentasClientes{
	...
	function crearReporte($datos,$formatos){
		require "eko_framework/app/models/reporte_ventas_clientes_pdf.php";
		
		$pdf=new ReporteVentasClientesPDF('P','mm','Letter',$datos,$formatos);
		$pdf->AddPage('P');
	...
	function obtenerDatos($params){
		...
		$filtroSql = " WHERE r.id_empresa = $idEmp AND r.fecha BETWEEN '$fechaInicio' AND '$fechaFin'";
		...
		$query = "SELECT 	DATE_FORMAT(r.fecha,'%d/%m/%Y') AS fecha, CONCAT(r.serie,' - ',r.folio) AS seriefolio, 
					CASE r.condicion_pago WHEN 1 THEN 'CONTADO' WHEN 2 THEN 'CREDITO' END AS condicion_pago,
					c.nombre_fiscal AS nombre_cliente,r.concepto,
					r.total
					FROM remisiones	r	
					INNER JOIN cat_clientes c ON c.id_cliente = r.id_cliente
					$filtroSql
					ORDER BY c.nombre_fiscal,r.fecha";
