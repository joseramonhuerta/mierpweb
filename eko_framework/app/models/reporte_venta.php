...
class ReporteVenta{
	...
	function crearReporte($datos,$formatos){
		...
		require "eko_framework/app/models/reporte_venta_pdf.php";
		
		$pdf=new ReporteVentaPDF('P','mm','Letter',$datos,$formatos);
		//--------------------------------------------------------------------------------------------------------
		$pdf->AddPage('P');
	...
	function obtenerDatos($params){
		...
		v.concepto_venta,v.importe,v.descuento,v.subtotal,v.impuestos,v.total,e.nombre_fiscal,s.nombre_sucursal,c.nombre_fiscal
		FROM ventas v 
		INNER JOIN cat_empresas e ON e.id_empresa = v.id_empresa
