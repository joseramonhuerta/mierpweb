...
class ReporteAbonoTicket{
	...
	function crearReporte($datos,$formatos){
		require "eko_framework/app/models/reporte_abono_ticket_pdf.php";
		
		$pdf=new ReporteAbonoTicketPDF('P','mm',array(250,70),$datos,$formatos);
		$pdf->AddPage();
	...
	function obtenerDatos($params){
        ...
        $query = "SELECT a.id_cxc_abono,a.serie,a.folio,a.concepto,DATE_FORMAT(a.fecha,'%d/%m/%y %H:%i') AS fecha,
		a.importe,s.nombre_sucursal,c.nombre_fiscal nombre_cliente,a.observacion
		FROM cxc_abonos a
		...
		INNER JOIN cat_clientes c ON c.id_cliente = cx.id_cliente
