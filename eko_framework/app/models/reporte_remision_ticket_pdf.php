...
class ReporteRemisionTicketPDF extends PDF_JavaScript{
	var $cerrando=false;
	var $colorHeader;
	...
	function ReporteRemisionTicketPDF($orientation='L',$unit='mm',$format='Letter',$datos,$formatos){
	...
	public function Header() {
		...
		$empresa=$datos['remision']['nombre_fiscal'];
		$sucursal=$datos['remision']['nombre_sucursal'];
		...
		$this->Cell($x3,5,utf8_decode("Reporte ".$nombre_movimiento),0,0,'L');	
		$this->Ln();
		$this->SetX($x2);
		//-------------------------------------------------
		...
		$this->SetX($x2);
		$this->SetFont($font,'B',8);
		$this->Cell(16,$hCell,"Sucursal:",$border,0,'L');	#	Valor
		$this->SetFont($font,'',9);
		$this->Cell(44,$hCell,mb_strtoupper(UTF8_decode($sucursal)),$border,0,'L');	#	Valor
					
		$this->Ln();
		$this->SetX($x2);
		$this->SetFont($font,'B',8);
		$this->Cell(16,$hCell,"Serie Folio: ",$border,0);							#	Label
		$this->SetFont($font,'',8);
		$this->Cell(44,$hCell,mb_strtoupper(UTF8_decode($serie.' - '.$folio)),$border,0);	#	Valor
				
		$this->Ln();
		$this->SetX($x2);
		$this->SetFont($font,'B',8);
		$this->Cell(16,$hCell,"Almacen:",$border,0,'L');	#	Valor
		$this->SetFont($font,'',8);
		$this->Cell(44,$hCell,mb_strtoupper(UTF8_decode($almacen)),$border,0,'L');	#	Valor
				
		$this->Ln();
		$this->SetX($x2);
		$this->SetFont($font,'B',8);
		$this->Cell(16,$hCell,"Vendedor:",$border,0,'L');	#	Valor
		$this->SetFont($font,'',8);
		$this->Cell(44,$hCell,mb_strtoupper(UTF8_decode($agente)),$border,0,'L');	#	Valor			
		...
		$this->SetFont($font,'',8);
		$this->Cell(42,$hCell,mb_strtoupper(UTF8_decode($fecha)),$border,0);	#	Valor
		
		$this->Ln();
		$this->SetX($x2);
		$this->SetFont($font,'B',8);	
		$this->Cell(25,$hCell,"Condicion Pago: ",$border,0);							#	Label
		$this->SetFont($font,'',8);
		$this->Cell(35,$hCell,mb_strtoupper(UTF8_decode($condicion_pago)),$border,0);	#	Valor
		...
		$this->SetFont($font,'',8);
		$this->Cell(42,$hCell,mb_strtoupper(UTF8_decode($aplicado)),$border,0);	#	Valor
		...
		$this->SetFont($font,'',8);
		$this->Cell(45,$hCell,mb_strtoupper(UTF8_decode($concepto)),$border,0);	#	Valor
	...
	function imprimeHeader(){
		...
		if ($this->imprimirHeader==true){
			...
			$this->Cell(12,4,"TOTAL",$border,0,"C",$fill);		//------Header TOTAL	
			$this->Ln();
			$this->SetX(2);	
