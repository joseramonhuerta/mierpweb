...
class ReporteMovimientoAlmacenCartaPDF extends FPDF{
	var $cerrando=false;
	var $colorHeader;
	...
	function ReporteMovimientoAlmacenCartaPDF($orientation='L',$unit='mm',$format='Letter',$datos,$formatos){
	...
	public function Header() {
		...
		$almacen_origen=$datos['movimiento']['almacen_origen'];
		$almacen_destino=$datos['movimiento']['almacen_destino'];
		$agente=$datos['movimiento']['nombre_agente'];
		$tipo_movimiento=$datos['movimiento']['tipo_movimiento'];
		$nombre_movimiento=$datos['movimiento']['nombre_movimiento'];
		$serie=$datos['movimiento']['serie_movimiento'];
		$folio=$datos['movimiento']['folio_movimiento'];
		$fecha=$datos['movimiento']['fecha_movimiento'];
		$concepto=$datos['movimiento']['concepto_movimiento'];
		$condicion_pago=$datos['movimiento']['condicion_pago'];
		...
		$this->SetY(10,10);	
		$this->Cell(200,5,utf8_decode("Reporte ".$nombre_movimiento),0,0,'L');	
		$this->Ln();
		$this->SetX($x2);
		//-------------------------------------------------
		...
		$this->Cell($x3,$hCell,mb_strtoupper(UTF8_decode($empresa.' ( '.$sucursal.' )')),$border,0);	#	Valor
		$this->Ln();
		...
		$this->SetFont($font,'B',9);		
		$this->Cell(15,$hCell,"Almacen Origen:",$border,0,'L');	#	Valor
		$this->SetFont($font,'',9);
		$this->Cell(60,$hCell,mb_strtoupper(UTF8_decode($almacen_origen)),$border,0,'R');	#	Valor		
		...
		$this->SetFont($font,'B',9);
		$this->Cell(15,$hCell,"Almacen Destino:",$border,0,'L');	#	Valor
		$this->SetFont($font,'',9);
		$this->Cell(60,$hCell,mb_strtoupper(UTF8_decode($almacen_destino)),$border,0,'R');	#	Valor				
		if($tipo_movimiento == 4){
			$this->Ln();
			$this->SetX($x2);
			$this->SetFont($font,'B',9);	
			$this->Cell(20,$hCell,"Vendedor: ",$border,0);							#	Label
			$this->SetFont($font,'',9);
			$this->Cell(175,$hCell,mb_strtoupper(UTF8_decode($agente)),$border,0);	#	Valor	
		}
		...
		$this->SetFont($font,'',9);
		$this->Cell(175,$hCell,mb_strtoupper(UTF8_decode($concepto)),$border,0);	#	Valor
