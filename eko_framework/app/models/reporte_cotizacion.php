...
class ReporteCotizacion{
	...
	function crearReporte($datos,$formatos){
		...
		require "eko_framework/app/models/reporte_cotizacion_pdf.php";
		
		$pdf=new ReporteCotizacionPDF('P','mm','Letter',$datos,$formatos);
		// --------------------------------------------------------------------------------------------------------
		$pdf->AddPage('P');
	...
	function obtenerDatos($params){
		...
		$query = "SELECT d.id_cotizacion_detalle,p.codigo,p.codigo_barras,p.descripcion,u.codigo_unidad,d.cantidad,d.costo,d.importe,d.descuento,d.subtotal,d.impuestos,d.total 
		FROM cotizaciones_detalles d 
		...
		$response=array();
      	$response['data']['cotizacion']=$resArr[0]; 
