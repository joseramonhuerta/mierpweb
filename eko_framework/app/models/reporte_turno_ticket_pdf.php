...
class ReporteTurnoTicketPDF extends PDF_JavaScript{
	var $cerrando=false;
	var $colorHeader;
	...
	function ReporteTurnoTicketPDF($orientation='L',$unit='mm',$format='Letter',$datos,$formatos){
	...
	public function Header() {
		...
		$consecutivo=$datos['turno']['consecutivo'];
		$fecha_turno=$datos['turno']['fecha'];
		
		$concepto=$datos['turno']['concepto'];
		//----------------------------------------------------
		//			Titulo del reporte
		//----------------------------------------------------
		...
		$this->Ln();
		$this->SetX(2);
		$this->Cell(80,$hCell,mb_strtoupper(UTF8_decode("APERTURA DE TURNO")),$border,0,'L');	#	Valor
		$this->Ln();
		$this->SetX(2);
	...
	function imprimirTotales($ultimo=false){
		...
		$total = number_format ($datos['turno']['total_turno'] ,$decimales ,  '.' , ',' );
