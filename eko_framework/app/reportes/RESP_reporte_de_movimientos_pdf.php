...
class ReporteDeMovimientosPDF extends TCPDF {
	...
	function getDatos(){
		...
		if($this->params["porSerie"]){ // REPORTE POR SERIES... (ANTONIO)
			...
			LEFT JOIN inventarios_movimientos_detalle ON KEYInvDetKar=IDInventarioDet
			...
			WHERE FechaKar BETWEEN '$fInicial' AND '$fFinal 23:59:59' AND FechaCancDet IS NULL AND KEYAlmacenKar IN ($idsAlmacenes) $filtroAlmacen
			GROUP BY IDKar
	...
	function ultimos($KEYProductoKar){
		...
		WHERE FechaKar < '".$this->params["fInicial"]."' AND KEYProductoKar =$KEYProductoKar GROUP BY KEYAlmacenKar";
