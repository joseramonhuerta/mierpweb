...
class KardexDeMovimientosPDF extends TCPDF {
	public function Header() {
       ...
       // $this->Image($image_file,$this->lMargin,15, 25,'', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
		...
		$this->Cell(0, 0, 'REPORTE DE EXISTENCIAS', 0, false, 'L', 0, '', 1, false, 'M', 'M');
		$this->ln();
		$this->setX($x);
	...
	function imprimir($path,$mode='I',$params){
		...
		//-------------------------------------------------------------------------------------------
		$sqlReporte="SELECT KEYAlmacenKar,KEYProductoKar,MAX(FechaKar) FechaKar
		...
		WHERE FechaKar<='$fFinal' AND FechaCancDet IS NULL AND KEYDetCancelado_MovDet=0 AND KEYTipoMovimientoDet!=3 GROUP BY KEYAlmacenKar, KEYProductoKar
		ORDER BY DescProd ASC, IDProd;";//Selecciona el ultimo registro con fecha menor o igual a la fecha superior del rango especificado
		//-------------------------------------------------------------------------------------------
		...
		foreach( $arrUltimos as $rec ){ 
			...
			IFNULL( IFNULL(CompraKar/CantidadEntradaKar,VentaKar/CantidadSalidaKar),0) as ultimo, FechaKar
			FROM inventarios_kardex 
			LEFT JOIN inventarios_movimientos_detalle ON IDInventarioDet=KEYInvDetKar
			LEFT JOIN cat_productos ON IDProd=KEYProductoKar 
